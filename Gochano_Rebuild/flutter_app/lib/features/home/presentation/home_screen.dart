...
//
// Order (spec §27): greeting → quick access → today's tasks → next medicine
// → commute → money.
//
...
import 'commute_trip_sheet.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return GochanoScaffold(
      ...
      body: ListView(
        ...
        children: [
          // 1. Quick Access — BEFORE the main Today card.
          SectionHeader(
          ...
          // 2. Today's tasks / assignments.
          _TodaysTasksCard(
            onSeeAll: () => onOpenDestination(_isStudent ? 1 : 2),
          ),
          const SizedBox(height: GochanoSpacing.sm),

          // 3. Medicine + Commute cards.
          _NextMedicineCard(
            onOpenMedicine: _isStudent
      ...
      builder: (context, medSnapshot) {
        ...
        return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, doseSnapshot) {
            final schedule = MedicineSchedule.forDay(medicines, [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(next.time, style: context.type.caption),
                      ],
                    ),
                  ),
                  if (overdue)
...
/// Shows the next planned trip, or a "Plan a trip" empty state.
///
/// Reads from the `commute_trips` collection to find the nearest future trip.
/// Tapping the empty state opens the trip planning sheet; tapping an existing
/// trip opens the edit sheet.
class _CommuteCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: FirestoreService.ownerStream('commute_trips', limit: 10),
      builder: (context, snapshot) {
        if (snapshot.hasError) return const SizedBox.shrink();
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const _SectionSkeleton();
        ...
        // Find the next future trip (by date).
        QueryDocumentSnapshot<Map<String, dynamic>>? nextTrip;
        for (final doc in docs) {
          final data = doc.data();
          final tripDate = (data['date'] as Timestamp?)?.toDate();
          if (tripDate == null) continue;
          if (tripDate.isAfter(now) || _isSameDay(tripDate, now)) {
            nextTrip = doc;
            break;
          }
        }
        ...
        if (nextTrip == null) {
          ...
          return AppCard(
            ...
            onTap: () => showCommuteTripSheet(context),
            child: Row(
              children: [
                GochanoIllustrationTile(
        ...
        // Show the next planned trip.
        final data = nextTrip.data();
        final origin = data['origin']?.toString() ?? '';
        final destination = data['destination']?.toString() ?? '';
        final tripDate = (data['date'] as Timestamp?)?.toDate();
        final leaveBy = data['leaveBy']?.toString();
        final reminderBefore = data['reminderBefore']?.toString();

        final dateLabel = tripDate != null ? _dateLabel(tripDate) : '';
        ...
        final routeLabel = origin.isNotEmpty && destination.isNotEmpty
            ...
            ? destination
            : GochanoLanguage.text('Trip planned', 'যাত্রা পরিকল্পিত');

        return AppCard(
          accent: colors.commute,
          onTap: () => showCommuteTripSheet(context, trip: nextTrip),
          child: Row(
            children: [
              GochanoIllustrationTile(
                GochanoArt.featureCommute,
                  ...
                  children: [
                    ...
                    Text(
                      [
                        ...
                        if (leaveBy != null && leaveBy.isNotEmpty)
                          GochanoLanguage.text(
                            'Leave by $leaveBy',
                            '$leaveBy এ বের হোন',
                          ),
                    ...
                    if (reminderBefore != null && reminderBefore.isNotEmpty)
                      Text(
                        GochanoLanguage.text(
                          'Reminder · $reminderBefore before',
                          'রিমাইন্ডার · $reminderBefore আগে',
                        ),
                        style: context.type.caption,
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
