...
void main() {
  ...
  group('StrategyChooser layout and IntrinsicHeight audit', () {
    test(
      'journey_view.dart does not use IntrinsicHeight or CrossAxisAlignment.stretch in _StrategyChooser',
      () {
        final file = File(
        ...
        final chooserSection = content
            .split('class _StrategyChooser')[1]
            .split('class ')[0];
        expect(
          chooserSection.contains('IntrinsicHeight'),
          isFalse,
          reason: '_StrategyChooser must never use IntrinsicHeight',
        );
        expect(
          chooserSection.contains('CrossAxisAlignment.stretch'),
          isFalse,
    ...
    testWidgets(
      'renders in an unconstrained scroll view without layout errors',
      (tester) async {
        ...
        await tester.pumpWidget(
          _wrap(JourneyPlanSection(plan: plan, hideMap: true)),
        );
  ...
  group('Planned commute trip notification policy', () {
    test(
      'generates deterministic positive integer notification id per tripId',
      () {
        final id1 = NotificationService.debugCommuteTripNotificationId(
          'trip_123',
        );
        final id2 = NotificationService.debugCommuteTripNotificationId(
          'trip_123',
        );
        final id3 = NotificationService.debugCommuteTripNotificationId(
          'trip_456',
        );
    ...
    test('10 min selection produces unique notification id', () {
      final id10 = NotificationService.debugCommuteTripNotificationId(
        'trip_x',
        10,
      );
      final id30 = NotificationService.debugCommuteTripNotificationId(
        'trip_x',
        30,
      );
      final id60 = NotificationService.debugCommuteTripNotificationId(
        'trip_x',
        60,
      );
    ...
    test(
      'same reminder minutes for different trips produces different IDs',
      () {
        final idA = NotificationService.debugCommuteTripNotificationId(
          'trip_a',
          10,
        );
        final idB = NotificationService.debugCommuteTripNotificationId(
          'trip_b',
          10,
        );
    ...
    test('produces stable pinned values from FNV-1a (not Dart .hashCode)', () {
      ...
      expect(
        idA,
        equals(NotificationService.debugCommuteTripNotificationId('trip_123')),
      );
      expect(
        idB,
        equals(NotificationService.debugCommuteTripNotificationId('abc')),
      );
      expect(
        idC,
        equals(NotificationService.debugCommuteTripNotificationId('')),
      );
    ...
    test(
      'notification_service.dart does not use .hashCode for commute reminder IDs',
      () {
    ...
    test(
      'all IDs are within valid Android notification range [0, 0x7fffffff]',
      () {
        const sampleIds = [
          'trip_test_1',
        ...
        for (final id in sampleIds) {
          ...
          final nid10 = NotificationService.debugCommuteTripNotificationId(
            id,
            10,
          );
    ...
    test(
      'notification_service.dart only schedules single reminder for commute trips',
      () {
        final file = File('lib/services/notification_service.dart');
        final content = file.readAsStringSync();
        ...
        expect(
          content.contains('_commuteTripReminderOffsets'),
          isFalse,
          reason:
              'Old multi-offset list _commuteTripReminderOffsets must be removed',
        );
      },
    );
  });
  ...
  group('PlanTripSheet completion surface', () {
    testWidgets('shows "I did the trip" button for upcoming trips', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(_wrap(PlanTripForm(existingTrip: trip)));
      await tester.pumpAndSettle();
    ...
    testWidgets('does not show "I did the trip" for completed trips', (
      tester,
    ) async {
      final departure = DateTime.now().add(const Duration(days: 2));
      final trip = PlannedCommuteTrip(
  ...
  group('PlanTripSheet edit and delete surface', () {
    testWidgets(
      'renders Create mode with Save button when existingTrip is null',
      (tester) async {
        await tester.pumpWidget(_wrap(const PlanTripForm()));
        await tester.pumpAndSettle();
    ...
    testWidgets(
      'renders Edit mode with Update, Delete, and completion buttons',
      (tester) async {
        final departure = DateTime.now().add(const Duration(days: 2));
        final trip = PlannedCommuteTrip(
          id: 'trip_edit_test',
  ...
  group('Firestore rules contract for planned_commute_trips', () {
    test(
      'firestore.rules contains owner-only rules for planned_commute_trips',
      () {
        ...
        if (rulesFile.existsSync()) {
          ...
          expect(
            rules.contains('allow read, delete: if ownedReadDelete();'),
            isTrue,
          );
  ...
  group('Commute fallback behavior and alternatives', () {
    test(
      'dataset_unavailable or plannerError yields non-null fallback plan without crash',
      () {
        final response = {
          'journeyPlanning': {
            'available': false,
    ...
    test(
      'outside_network_coverage correctly captures off-network endpoints',
      () {
        final response = {
          'journeyPlanning': {
            'available': false,
    ...
    testWidgets(
      'renders unavailable explanation banner instead of blocking network crash',
      (tester) async {
        final plan = JourneyPlan.fromResponse({
          'journeyPlanning': {
            'available': false,
    ...
    testWidgets(
      'displays up to 3 alternatives in StrategyChooser and updates selection on tap',
      (tester) async {
        int selectedIdx = 0;
        final plan = JourneyPlan.fromResponse({
          'journeyPlanning': {'available': true},
